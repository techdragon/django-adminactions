...
def export_as_xls2(queryset, fields=None, header=None, filename=None, options=None, out=None):
    ...
    sheet.write(row, 0, force_str('#'), style)
    if header:
        if not isinstance(header, (list, tuple)):
            header = [force_str(f.verbose_name) for f in queryset.model._meta.fields if f.name in fields]

        for col, fieldname in enumerate(header, start=1):
            sheet.write(row, col, force_str(fieldname), heading_xf)
